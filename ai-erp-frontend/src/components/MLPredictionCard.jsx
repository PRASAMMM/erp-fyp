...
import { getStudentPrediction } from '../services/api';

// ── Risk badge colours ────────────────────────────────────────────────────────
const RISK_CONFIG = {
  HIGH:    { bg: 'rgba(220, 38, 38, 0.1)', border: 'rgba(220, 38, 38, 0.2)', text: '#ef4444', dot: '#ef4444', label: 'High Risk' },
  MEDIUM:  { bg: 'rgba(217, 119, 6, 0.1)', border: 'rgba(217, 119, 6, 0.2)', text: '#f59e0b', dot: '#f59e0b', label: 'Medium Risk' },
  LOW:     { bg: 'rgba(22, 163, 74, 0.1)', border: 'rgba(22, 163, 74, 0.2)', text: '#10b981', dot: '#10b981', label: 'Low Risk' },
  UNKNOWN: { bg: 'rgba(107, 114, 128, 0.1)', border: 'rgba(107, 114, 128, 0.2)', text: 'var(--text-muted)', dot: 'var(--text-muted)', label: 'Unknown' },
};

const GRADE_CONFIG = {
  A: { color: '#10b981', desc: 'Excellent' },
  B: { color: '#3b82f6', desc: 'Good' },
  C: { color: '#f59e0b', desc: 'Average' },
  D: { color: '#f97316', desc: 'Below Average' },
  F: { color: '#ef4444', desc: 'Failing' },
  'N/A': { color: 'var(--text-muted)', desc: 'N/A' },
};
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
      <svg width={size} height={size}>
        ...
        <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle"
          fontSize={size < 80 ? 13 : 16} fontWeight="600" fill="currentColor">
          {Math.round(value)}
        </text>
      </svg>
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between',
        ...
        <span>{label}</span>
        <span style={{ fontWeight: 600, color: 'currentColor' }}>{Math.round(value)}{max === 100 ? '%' : ''}</span>
      </div>
...
export default function MLPredictionCard({ studentId }) {
  ...
  if (loading) return (
    <div className="card">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <span style={{ color: 'var(--text-muted)', fontSize: 14 }}>Generating AI predictions…</span>
      </div>
    </div>
  );
  ...
  if (error || !data) return (
    <div className="card" style={{ borderColor: 'var(--danger)', background: 'rgba(220, 38, 38, 0.05)' }}>
      <p style={{ color: 'var(--danger)', fontSize: 13 }}>
        ML service unavailable. Start Flask: <code>python app.py</code>
      </p>
    </div>
  );
  ...
  return (
    <div className="card">
        ...
        <>
          <button
            ...
            style={{
              ...
              fontSize: 12, color: 'var(--text-muted)', padding: 0, marginBottom: 8,
      ...
      {/* Footer */}
      <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '10px 0 0', textAlign: 'right' }}>
